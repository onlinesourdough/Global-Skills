...
- Keep the root `.codex-plugin/plugin.json` thin: it exposes the same
  ...
  or run-history data.
- Treat `clarify`, `manage-skills`, `orchestrate-workers`, and `route-models`
  as optional global capabilities. System, Project, domain, lifecycle, and
  audit skills remain with their existing owners.
